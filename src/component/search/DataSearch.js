  ...
  const [formData, setFormData] = useState({
    ...
    const handleDown = async() => {
      if (formData.type === 'matching') {
        await downRef.current.downloadMatching();
      } else if (formData.type === 'priceCheck' && formData.dataType === 'getData') {
        await downRef.current.downloadPriceGet();
      } else if (formData.type === 'sendCheck') {
        await downRef.current.downloadSendCheck();
      } else if (formData.type === 'rocket') {
        await downRef.current.downloadRocket();
      } else if (formData.type === 'bulk') {
        await downRef.current.downloadBulk();
      } else {
        await downRef.current.downloadPriceSend();
      }
    }
  ...
  return (
    <>
     ...
     <Suspense fallback={<div><Loading /></div>}>
      ...
      <Download formData={formData} ref={downRef} />
     </Suspense>
      <div>
        {loading === true ? 
        <Loading /> : null}
